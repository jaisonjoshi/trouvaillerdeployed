...
function HotelCard({itm, k}) {
  ...
  return (
    <div
      className="mb-4 w-full h-[100%] pb-3 card-shadow-1 rounded-lg  cursor-pointer"
    ...
    >
      ...
      <div className="imagegradient absolute hidden sm:block top-0  w-[100%] bottom-0 z-[100] rounded "></div>
        {itm.images && itm.images.length !== 0 && <img
          ...
          alt=""
        />}
        <h3 className="hidden sm:block  sm:text-base md:text-xl sm:font-bold z-[101] text-whiteglow px-1 md:px-3  absolute bottom-[3px] md:bottom-[10px]">
          {itm.title}
        </h3>
      </div>

      <h3 className="text-base z-50 pt-5  mx-2 md:mx-3 text-[black] font-semibold sm:hidden  card-text-heading ">
        {itm.title}
      </h3>
